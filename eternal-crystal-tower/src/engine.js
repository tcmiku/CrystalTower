...
import { FORMATIONS, REACTIONS, WEAKPOINTS, WEAKPOINT_RULES, sectorAngle } from "./chapter-one.js";
...
export function createGameState(seed = 1, research = { damage: 0, health: 0, income: 0 }, relicUnlocks = { ward: true }, relicSlots = GAME_CONFIG.relics.initialSlots, relicArchive = {}, equippedSeals = [], skillResearch = {}, chapter = 1) {
  ...
  const state = {
    ...
    tower: {
      ...
      anchorLockTimer: 0,
      priorityTargetIds: [],
      gunAimTargetIds: { base: null, pulse: null, cannon: null },
      siegeTargetId: null,
      siegeStreak: 0,
      cannonEchoChain: 0,
    ...
    skills: {
      ...
      overload: { cooldown: 0, active: 0, heat: 0, slow: 0, pulse: 0, overheated: false, permanentEngaged: false, unstable: 0 },
...
export function getTowerStats(state) {
  ...
  const shopDamage = 1 + (state.endlessShop?.levels?.attackProtocol ?? 0) * 0.12;
...
export function getTechStatus(state, key) {
  ...
  if (state.tower.moduleBay && key === "rate" && !["pulse", "cannon", "mortar"].some(id => installedModule(state, id))) return { unlocked: false, maxed: false, cost: getUpgradeCost(state, key), reason: "需要主炮模块" };
...
export function toggleDroneMode(state) {
  ...
  if (state.tower.droneMode === "attack" && state.tower.droneEnergy <= getDroneEnergyMax(state) * 0.5 && modulesAdjacent(state, "hangar", "pulse") && !(state.tower.relayCooldown > 0)) {
    state.tower.pulseRelay = 6;
    state.tower.relayCooldown = 12;
    state.events.push({ type: "moduleSynergy", name: "回航接力 · 轻炮加速 6 秒" });
  }
  ...
  if (service) {
    service.launchBuff = state.tower.droneMode === "attack" && service.level >= 3 && service.recharged ? MODULE_BALANCE.service.launchDuration : 0;
    if (state.tower.droneMode === "attack") { service.recharged = false; service.recoveredEnergy = 0; }
    if (service.launchBuff > 0) state.events.push({ type: "moduleServiceLaunch" });
  }
...
function chapterOneRewardChoices(state) {
  const limit = 3 + (state.threatSeals?.modifiers?.relicChoiceBonus ?? 0);
  const specs = Object.keys(SPECIALIZATIONS).filter(id => {
    const module = installedModule(state, SPECIALIZATIONS[id].module);
    return module && !module.specialization;
  });
  const specialization = specs.length ? `spec:${specs[Math.floor(state.rng.next() * specs.length)]}` : null;
  const relics = buildRelicChoices(state).filter(id => !id.startsWith("boost:"));
  const choices = relics.slice(0, limit - (specialization ? 2 : 1));
  if (specialization) choices.push(specialization);
  choices.push(state.tower.hp < getTowerStats(state).maxHp * 0.8 ? "supply:repair" : "supply:coins");
  if (choices.length < limit && !choices.includes("supply:repair")) choices.push("supply:repair");
  if (choices.length < limit && !choices.includes("supply:coins")) choices.push("supply:coins");
  return choices;
}
...
export function offerRelicChoice(state, source = "eliteWave") {
  ...
  const choices = source === "endlessWave" ? [ENDLESS_RELIC_ID] : state.tower.moduleBay ? chapterOneRewardChoices(state) : buildRelicChoices(state);
  if (!choices.length || (!state.tower.moduleBay && state.relics.picks >= state.relics.slots && !choices.some((id) => id.startsWith("boost:")))) return false;
...
export function lockRelicChoice(state, id) {
  if (!state.relicChoice || !state.relicChoice.choices.includes(id) || !MECHANIC_RELIC_IDS.includes(id)) return false;
...
export function chooseRelic(state, id, replaceId = null) {
  if (!state.relicChoice || !state.relicChoice.choices.includes(id)) return false;
  if (id.startsWith("spec:")) {
    if (!specializeModule(state, id.slice(5))) return false;
  } else if (id.startsWith("supply:")) {
    if (id === "supply:repair") {
      const stats = getTowerStats(state);
      const amount = stats.maxHp * 0.3;
      const healed = Math.min(amount, stats.maxHp - state.tower.hp);
      state.tower.hp += healed;
      state.tower.shield = Math.min(stats.maxHp * GAME_CONFIG.skills.heal.shieldCapFraction, state.tower.shield + amount - healed);
    } else if (id === "supply:coins") state.coins += 80 + state.threat * 12;
    else return false;
  } else if (id.startsWith("boost:")) {
    ...
    if (!MECHANIC_RELIC_IDS.includes(id) || state.relics.owned[id]) return false;
    if (state.relics.picks >= state.relics.slots) {
      if (!state.tower.moduleBay || !state.relics.owned[replaceId]) return false;
      state.relics.owned[replaceId] = false;
      if (replaceId === "mirror") state.relics.mirrorShots = 0;
      if (replaceId === "ward") state.relics.wardKills = 0;
      if (replaceId === "lunar") state.relics.phaseBuff = 0;
      if (replaceId === "decoy") state.decoys.length = 0;
      if (replaceId === "ember" || replaceId === "frostfire") state.emberZones.length = 0;
    } else state.relics.picks += 1;
...
export function findTargets(state, count = 1) {
  ...
  return rankTargets(state, candidates, count);
}

function rankTargets(state, candidates, limit = candidates.length) {
  const { x: centerX, y: centerY } = getTowerPosition(state);
  const towerRadius = getTowerRadius(state);
  const protocol = state.tower.targetProtocol;
  const lockActive = state.tower.anchorLockTimer > 0;
  ...
  const compare = (a, b) => {
    ...
    if (protocol === "hunter") {
      const threat = b.hunter - a.hunter;
      if (threat) return threat;
    } else if (protocol === "breach") {
      const breach = a.contact - b.contact;
      if (Math.abs(breach) > 0.0001) return breach;
    } else if (protocol === "radar") {
      const ranged = b.radar - a.radar;
      if (ranged) return ranged;
    }
    return a.distance - b.distance || a.enemy.id - b.enemy.id;
  };
  const safeLimit = Math.max(0, Math.min(ranked.length, limit));
  if (safeLimit === 0) return [];
  if (safeLimit >= ranked.length) return ranked.sort(compare).map((entry) => entry.enemy);
  const selected = [];
  for (const entry of ranked) {
    if (selected.length === safeLimit && compare(entry, selected[selected.length - 1]) >= 0) continue;
    let index = selected.length;
    while (index > 0 && compare(entry, selected[index - 1]) < 0) index -= 1;
...
const EMPTY_SPATIAL_QUERY = Object.freeze([]);

function buildEnemySpatialIndex(enemies) {
  const rows = new Map();
  let maxRadius = 0;
  for (let index = 0; index < enemies.length; index += 1) {
    ...
    if (!row) { row = new Map(); rows.set(cellY, row); }
    let bucket = row.get(cellX);
    if (!bucket) { bucket = []; row.set(cellX, bucket); }
    bucket.push(index);
  }
  return { rows, maxRadius };
}

function queryEnemySpatialIndex(index, x, y, radius = 0) {
  if (!index || index.rows.size === 0) return EMPTY_SPATIAL_QUERY;
  const reach = radius + index.maxRadius;
  const minX = Math.floor((x - reach) / ENEMY_SPATIAL_CELL_SIZE);
  const maxX = Math.floor((x + reach) / ENEMY_SPATIAL_CELL_SIZE);
  const minY = Math.floor((y - reach) / ENEMY_SPATIAL_CELL_SIZE);
  const maxY = Math.floor((y + reach) / ENEMY_SPATIAL_CELL_SIZE);
  let candidates = null;
  for (let cellY = minY; cellY <= maxY; cellY += 1) {
    const row = index.rows.get(cellY);
    if (!row) continue;
  ...
  if (!candidates) return EMPTY_SPATIAL_QUERY;
  if (candidates.length > 1) candidates.sort((a, b) => a - b);
  return candidates;
}

function fireStarPiercer(state, target, damage, muzzle = null) {
  const cfg = GAME_CONFIG.cannon.siege;
  const towerPos = getTowerPosition(state);
  const origin = muzzle ?? towerPos;
  const x = origin.x;
  const y = origin.y;
  ...
  if (hasEndlessRelic(state, "omniversalPiercer") && !target.elite && !isBossEnemy(target) && target.hp < 0) {
    ...
    const next = state.enemies
      ...
      .map((enemy) => {
        const offsetX = enemy.x - x;
        const offsetY = enemy.y - y;
        return { enemy, forward: offsetX * nx + offsetY * ny, lateral: Math.abs(offsetX * ny - offsetY * nx) };
      })
      .filter((candidate) => candidate.forward > length && candidate.lateral <= 70 + candidate.enemy.radius)
      .sort((a, b) => a.forward - b.forward || a.enemy.id - b.enemy.id)[0];
    if (next) {
      damageEnemy(state, next.enemy, overflow, "starPiercer");
      state.elementFx.push({ element: "starPiercer", x1: target.x, y1: target.y, x2: next.enemy.x, y2: next.enemy.y, life: cfg.starPiercerDuration, maxLife: cfg.starPiercerDuration });
      state.events.push({ type: "cannonStarPiercerOverflow", fromId: target.id, targetId: next.enemy.id, damage: overflow, x1: target.x, y1: target.y, x2: next.enemy.x, y2: next.enemy.y });
    }
  }
...
function fireTower(state, weaponId = null, claimedTargetIds = null) {
  ...
  if (module) {
    ...
    if (heavy && modulesAdjacent(state, "shield", "cannon")) stats.damage *= 1 + Math.min(3, state.tower.moduleShieldCharge ?? 0) * 0.2;
    if (!heavy) { stats.pierce = 0; stats.bossDamageMultiplier = 1; }
  }
  const position = getTowerPosition(state);
  const gunKey = weaponId ?? "base";
  const candidates = module ? state.enemies.filter((enemy) => {
  ...
  let targets = rankTargets(state, candidates, stats.projectileCount);
  if (claimedTargetIds && claimedTargetIds.size > 0) {
    const unclaimed = rankTargets(state, candidates.filter((enemy) => !claimedTargetIds.has(enemy.id)), stats.projectileCount);
    if (unclaimed.length) targets = unclaimed;
  }
  for (const target of targets) claimedTargetIds?.add(target.id);
  ...
  if (heavy) state.tower.moduleShieldCharge = 0;
  const { x: centerX, y: centerY } = getTowerPosition(state);
  ...
  for (let targetIndex = 0; targetIndex < targets.length; targetIndex += 1) {
    ...
    state.projectiles.push({
      id: state.nextId++, x: muzzle.x, y: muzzle.y,
      ...
      splitLevel: module ? Number(module.specialization === "pulseSplit") : state.tower.upgrades.cannonSplit,
      growthLevel: module ? Number(module.specialization === "pulseSplit") : state.tower.upgrades.cannonGrowth,
      splash: module?.specialization === "cannonBurst" ? 95 : 0,
...
export function damageEnemy(state, enemy, damage, source = "shot") {
  ...
  if ((enemy.fractureTimer ?? 0) > 0) appliedDamage *= REACTIONS.shatter.vulnerability;
  if ((enemy.formationGuard ?? 0) > 0 && source !== "starPiercer") appliedDamage *= 0.65;
  ...
  if (enemy.weakpointRole && appliedDamage > 0) {
  ...
  if (!["conduct", "resonance"].includes(source) && (enemy.conductTimer ?? 0) > 0 && appliedDamage > 0) {
    for (const linkedId of enemy.conductIds ?? []) {
      ...
      damageEnemy(state, linked, Math.min(hpBefore, appliedDamage) * REACTIONS.conduct.share, "conduct");
      state.elementFx.push({ element: "lightning", x1: enemy.x, y1: enemy.y, x2: linked.x, y2: linked.y, life: 0.16, maxLife: 0.16 });
    }
  }
  ...
  if (enemy.type === "sovereign" && enemy.hp > 0 && enemy.healthBar === GAME_CONFIG.sovereign.enrageHealthBar && !enemy.enraged) {
    enemy.enraged = true;
    enemy.elementImmune = !state.tower.moduleBay;
    enemy.freezeTimer = 0;
    enemy.burnTimer = 0;
    enemy.burnDamagePerTick = 0;
...
export function applyElementalHit(state, enemy, element, baseDamage) {
  ...
  if (state.tower.moduleBay) {
    ...
    if (prior && prior !== element && (enemy.moduleElementTimer ?? 0) > 0) {
      ...
      const reaction = pair === "fire+frost" ? "melt" : pair === "fire+lightning" ? "conduct" : "shatter";
      const rule = REACTIONS[reaction];
      enemy.moduleElement = null;
      enemy.moduleElementTimer = 0;
      let targets = state.enemies.filter(target => target.hp > 0 && Math.hypot(target.x - enemy.x, target.y - enemy.y) <= rule.radius + target.radius)
        .sort((a, b) => Math.hypot(a.x - enemy.x, a.y - enemy.y) - Math.hypot(b.x - enemy.x, b.y - enemy.y) || a.id - b.id);
      if (reaction !== "melt") targets = [enemy, ...targets.filter(target => target !== enemy).slice(0, rule.targets - 1)];
      if (reaction === "conduct") {
        for (const target of targets) {
          target.conductTimer = rule.duration;
          target.conductIds = targets.filter(other => other !== target).map(other => other.id);
        }
      } else {
        for (const target of targets) {
          if (reaction === "shatter") target.fractureTimer = rule.duration;
          damageEnemy(state, target, baseDamage * rule.damage, element);
        }
      }
      state.elementFx.push({ element: reaction === "shatter" ? "frost" : "cannonEcho", x: enemy.x, y: enemy.y, radius: rule.radius, life: 0.5, maxLife: 0.5 });
      state.floaters.push({ x: enemy.x, y: enemy.y - enemy.radius, text: rule.name, life: 1, color: reaction === "melt" ? "#ffae78" : reaction === "shatter" ? "#91ddff" : "#d3acff" });
      state.events.push({ type: "moduleReaction", reaction: rule.name, hits: targets.length, enemyId: enemy.id });
    } else {
  ...
  if (element === "frost") {
    if (state.tower.moduleBay && isBossEnemy(enemy) && enemy.enraged) {
      enemy.freezeTimer = 0;
      enemy.fractureTimer = Math.max(enemy.fractureTimer ?? 0, 2);
      state.events.push({ type: "bossBrittle", enemyId: enemy.id });
      return true;
    }
...
function triggerCannonCascade(state, enemy) {
  ...
  for (const target of state.enemies) {
    ...
    if (hasEndlessRelic(state, "frostRift") && !(target.type === "colossus" && target.enraged) && !(target.type === "sovereign" && target.elementImmune)) {
      const duration = isBossEnemy(target) ? 0.25 : target.elite ? 0.6 : 1;
      target.freezeTimer = Math.max(target.freezeTimer ?? 0, duration);
      target.frostCascadeFrozen = true;
    }
...
function resolveDeaths(state) {
  ...
  let frostCascadeBloomBudget = ENDLESS_SHOP_RULES.frostCascadeChainLimit;
  for (const enemy of state.enemies) {
    if (enemy.hp > 0 || enemy.deadHandled) continue;
    enemy.deadHandled = true;
    if (enemy.type === "anchor") {
      if (enemy.weakpointRole && !enemy.weakpointCollapsed) {
        const boss = state.enemies.find(target => target.id === enemy.anchorBossId && target.hp > 0);
        if (boss) {
          for (const other of state.enemies) if (other !== enemy && other.weakpointRole && other.anchorBossId === boss.id) { other.hp = 0; other.weakpointCollapsed = true; }
          boss.weakpointCooldown = WEAKPOINT_RULES.interval;
          if (enemy.weakpointRole === "chain") damageEnemy(state, boss, boss.maxHp * WEAKPOINT_RULES.feedbackFraction, "resonance");
          if (enemy.weakpointRole === "armor") boss.fractureTimer = WEAKPOINT_RULES.expose;
          if (enemy.weakpointRole === "relay") {
            boss.objectiveStun = WEAKPOINT_RULES.stun;
            boss.activeSkill = null;
            boss.activeSkills = {};
            boss.intentSkill = null;
            boss.skillCooldown = 1;
            state.hostileProjectiles.length = 0;
            state.summonRifts = state.summonRifts.filter(rift => rift.bossId !== boss.id);
          }
          state.events.push({ type: "bossWeakpointBroken", role: enemy.weakpointRole, enemyId: boss.id });
        }
      }
      if (enemy.riftAnchor) {
    ...
    const frostCascadeBloomAllowed = !enemy.frostCascadeFrozen || frostCascadeBloomBudget > 0;
    if (state.relics.owned.frostbloom && (enemy.freezeTimer ?? 0) > 0 && frostCascadeBloomAllowed) {
      if (enemy.frostCascadeFrozen) frostCascadeBloomBudget -= 1;
...
export function updateChapterOneObjectives(state, dt) {
  if (!state.tower.moduleBay) return;
  for (const boss of state.enemies.filter(enemy => ["colossus", "sovereign"].includes(enemy.type) && enemy.hp > 0)) {
    if ((boss.entryTimer ?? 0) > 0 || (boss.phaseBreakInvulnerability ?? 0) > 0) continue;
    boss.weakpointCooldown = Math.max(0, (boss.weakpointCooldown ?? 0) - dt);
    if (boss.weakpointCooldown > 0 || state.enemies.some(enemy => enemy.weakpointRole && enemy.anchorBossId === boss.id && enemy.hp > 0)) continue;
    const tower = getTowerPosition(state);
    for (const [role, meta] of Object.entries(WEAKPOINTS)) {
      const radius = meta.radius * getTowerScale(state);
      const anchor = spawnEnemy(state, "anchor", { x: tower.x + Math.cos(meta.angle) * radius, y: tower.y + Math.sin(meta.angle) * radius }, { anchorBossId: boss.id, anchorRole: role === "relay" ? "summon" : role === "chain" ? "overload" : "shield" });
      if (!anchor) continue;
      anchor.weakpointRole = role;
      anchor.hp = anchor.maxHp = Math.max(60, boss.maxHp * WEAKPOINT_RULES.hpFraction);
      anchor.radius = 22;
      anchor.reward = 0;
    }
    boss.weakpointCooldown = WEAKPOINT_RULES.interval;
    state.events.push({ type: "bossWeakpoints", enemyId: boss.id });
  }
  const validBosses = new Set(state.enemies.filter(enemy => ["colossus", "sovereign"].includes(enemy.type) && enemy.hp > 0).map(enemy => enemy.id));
  state.enemies = state.enemies.filter(enemy => !enemy.weakpointRole || validBosses.has(enemy.anchorBossId));
}

function chapterOneWavePosition(state, sector) {
  const angle = sectorAngle(sector) + (state.rng.next() - 0.5) * Math.PI / 4;
  return getArenaEdgePosition(angle, state.rng.next() * GAME_CONFIG.arena.spawnRing.radialJitter);
}
...
function updateWave(state, dt) {
  ...
  if (!wave.active && state.time >= wave.nextAt) {
    ...
    if (state.tower.moduleBay) wave.remaining = Math.max(8, Math.ceil(wave.remaining * (FORMATIONS[wave.formation]?.countMultiplier ?? 1)));
    wave.spawned = 0;
    wave.spawnTimer = 0;
    wave.index += 1;
  ...
  while (wave.remaining > 0 && wave.spawnTimer <= 0) {
    ...
    const elite = state.endlessMode
      ...
      : wave.elitePending;
    const formation = state.tower.moduleBay ? FORMATIONS[wave.formation] : null;
    const sector = formation && wave.formation === "pincer" && wave.spawned % 2 ? (wave.direction + 2) % 6 : wave.direction;
    const type = formation ? formation.types[wave.spawned % formation.types.length] : chooseEnemyType(state);
    const position = formation ? chapterOneWavePosition(state, sector) : spawnPosition(state.rng, side);
    const spawned = spawnEnemy(state, type, position, { elite, waveElite: elite, waveIndex: wave.index });
    if (spawned && formation) {
      spawned.formation = wave.formation;
      spawned.formationFront = wave.formation === "wall" && type === "brute";
      if (wave.formation === "brood" && wave.spawned === 0) {
        spawned.broodRemaining = 4;
        spawned.broodTimer = 6;
        spawned.speed *= 0.5;
      }
      if (wave.formation === "battery" && spawned.attackRange > 0) spawned.volleyAt = Math.ceil((state.time + 2) / 6) * 6;
    }
    wave.spawned += 1;
    if (elite && spawned) {
...
export function damageTower(state, damage, heavy = false, source = "enemy", origin = null) {
  ...
  if (sector) {
    ...
    if (damage > 0 && !(state.tower.synergyCooldown > 0)) {
      ...
      if (activated) {
        state.tower.synergyCooldown = 1.2;
        state.events.push({ type: "moduleSynergy", name: "护盾接力 · 邻接武器充能" });
      }
    }
  }
  ...
  if (state.tower.hp <= 0 && hasEndlessRelic(state, "finalInsurance") && state.endlessShop.insuranceCharges > 0) {
    const maxHp = getTowerStats(state).maxHp;
    state.endlessShop.insuranceCharges -= 1;
    state.tower.hp = Math.max(1, maxHp * ENDLESS_SHOP_RULES.insuranceHealFraction);
    state.tower.shield += maxHp * ENDLESS_SHOP_RULES.insuranceShieldFraction;
    state.tower.damageImmunity = ENDLESS_SHOP_RULES.insuranceImmunity;
    state.events.push({ type: "endlessInsurance", hp: state.tower.hp, shield: maxHp * ENDLESS_SHOP_RULES.insuranceShieldFraction, immunity: state.tower.damageImmunity });
  }
...
function updateModuleSupport(state, dt) {
  const bay = state.tower.moduleBay;
  if (!bay) return;
  const combat = bay.combat;
  const tower = getTowerPosition(state);
  for (const effect of combat.effects) effect.life -= dt;
  combat.effects = combat.effects.filter(effect => effect.life > 0);
  ...
  if (anchor && !(anchor.facingCooldown > 0)) {
    ...
    if (anchor.cooldown <= 0 && state.enemies.some(enemy => enemy.hp > 0 && enemy.type !== "anchor" && Math.hypot(enemy.x - x, enemy.y - y) <= radius + enemy.radius)) {
      const duration = cfg.duration[anchor.level - 1];
      combat.fields.push({ x, y, radius, life: duration, duration });
      anchor.cooldown = cfg.interval;
      state.events.push({ type: "moduleGravity", x, y });
    }
  }
  for (const enemy of state.enemies) {
    enemy.gravitySlow = 1;
    if (enemy.hp <= 0 || enemy.type === "anchor") continue;
    for (const field of combat.fields) {
      const dx = field.x - enemy.x, dy = field.y - enemy.y, distance = Math.hypot(dx, dy);
      if (distance > field.radius + enemy.radius) continue;
  ...
  if (interceptor) {
    const capacity = rule.capacity[interceptor.level - 1], interval = rule.recharge[interceptor.level - 1];
    interceptor.charges ??= 0;
    if (interceptor.charges < capacity) {
      interceptor.recharge = (interceptor.recharge ?? 0) + dt;
      while (interceptor.recharge + 1e-9 >= interval && interceptor.charges < capacity) {
        interceptor.recharge -= interval;
        interceptor.charges += 1;
      }
    }
    if (interceptor.charges >= capacity) interceptor.recharge = 0;
  }
}

function updateMortar(state, dt) {
  const bay = state.tower.moduleBay;
  if (!bay) return;
  const combat = bay.combat, cfg = MODULE_BALANCE.mortar;
  for (const shell of combat.shells) {
    shell.life -= dt;
    if (shell.life > 0) continue;
    let hits = 0;
    for (const enemy of state.enemies) {
      if (enemy.hp <= 0 || Math.hypot(enemy.x - shell.x, enemy.y - shell.y) > shell.radius + enemy.radius) continue;
      damageEnemy(state, enemy, shell.damage, "explosion");
      if (shell.element) applyElementalHit(state, enemy, shell.element, shell.damage);
      if (shell.stagger && enemy.attackRange > 0 && !isBossEnemy(enemy) && enemy.type !== "anchor") {
        enemy.moduleSuppression = cfg.staggerDuration;
        enemy.attackCooldown = Math.max(enemy.attackCooldown, cfg.staggerDuration);
        if (enemy.volleyAt != null) enemy.volleyAt = Math.ceil((state.time + cfg.staggerDuration) / 6) * 6;
      }
      hits += 1;
    }
    combat.effects.push({ kind: "mortar", x: shell.x, y: shell.y, radius: shell.radius, life: .6, duration: .6 });
    state.events.push({ type: "mortarImpact", x: shell.x, y: shell.y, hits });
  }
  combat.shells = combat.shells.filter(shell => shell.life > 0);
  const mortar = installedModule(state, "mortar");
  if (!mortar) { state.tower.gunAimTargetIds.mortar = null; return; }
  mortar.cooldown = Math.max(0, (mortar.cooldown ?? cfg.interval) - dt);
  const tower = getTowerPosition(state), stats = getTowerStats(state);
  const candidates = state.enemies.filter(enemy => {
    const distance = Math.hypot(enemy.x - tower.x, enemy.y - tower.y);
    return enemy.hp > 0 && distance >= cfg.minRange && distance <= cfg.range;
  });
  const target = rankTargets(state, candidates, 1)[0];
  state.tower.gunAimTargetIds.mortar = target?.id ?? null;
  ...
  for (let i = 0; i < (cluster ? 3 : 1); i++) {
    ...
    combat.shells.push({ id: state.nextId++, fromX: origin.x, fromY: origin.y,
      x: target.x + (cluster ? Math.cos(angle) * cfg.clusterSpread : 0), y: target.y + (cluster ? Math.sin(angle) * cfg.clusterSpread : 0),
      radius: cfg.radius[mortar.level - 1], life: flight, duration: flight, damage, element: rollProjectileElement(state, 1, "mortar"), stagger });
  }
...
function updateHostileProjectiles(state, dt) {
  ...
  for (const projectile of state.hostileProjectiles) {
    ...
    const reachedTarget = Math.hypot(oldX - projectile.targetX, oldY - projectile.targetY) <= Math.hypot(stepX, stepY) + projectile.radius + 9;
    ...
    if (projectile.kind === "enemyBolt") {
      if (reachedTarget || hitTower) {
        const decoy = projectile.decoyId ? state.decoys.find(item => item.id === projectile.decoyId && item.hp > 0) : null;
...
function updateEnemies(state, dt) {
  ...
  for (const enemy of state.enemies) {
    ...
    enemy.moduleSuppression = Math.max(0, (enemy.moduleSuppression ?? 0) - dt);
    enemy.moduleElementTimer = Math.max(0, (enemy.moduleElementTimer ?? 0) - dt);
    enemy.fractureTimer = Math.max(0, (enemy.fractureTimer ?? 0) - dt);
    enemy.conductTimer = Math.max(0, (enemy.conductTimer ?? 0) - dt);
    if (!enemy.conductTimer) enemy.conductIds = [];
    ...
    if ((enemy.objectiveStun ?? 0) > 0) {
      enemy.objectiveStun = Math.max(0, enemy.objectiveStun - dt);
      continue;
    }
    if (enemy.type === "colossus") {
    ...
    if (distance > targetRadius + enemy.radius + 3 + (enemy.attackRange ?? 0)) {
      ...
      if (enemy.volleyAt != null) {
        ...
        enemy.volleyAt = Math.ceil((state.time + 0.1) / 6) * 6;
        enemy.attackCooldown = 0;
      }
      if (enemy.attackCooldown <= 0) {
        ...
        const damage = enemy.damage * GAME_CONFIG.combat.enemyAttackInterval * (enemy.volleyAt != null ? 3 : 1);
        const heavy = isBossEnemy(enemy) || enemy.type === "brute" || enemy.type === "rammer" || enemy.type === "rustBeetle" || enemy.type === "porcelainWarden";
        if (state.tower.moduleBay && enemy.attackRange > 0 && !isBossEnemy(enemy)) {
          const angle = Math.atan2(targetY - enemy.y, targetX - enemy.x);
          const speed = MODULE_BALANCE.interceptor.projectileSpeed;
          state.hostileProjectiles.push({ id: state.nextId++, kind: "enemyBolt", interceptable: true,
            x: enemy.x, y: enemy.y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
            targetX, targetY, decoyId: decoy?.id ?? null, radius: 6, life: distance / speed + 1, damage, heavy, source: enemy.type });
          state.events.push({ type: "enemyShoot", x: enemy.x, y: enemy.y });
        } else if (decoy) {
...
function updateLaunchedSaws(state, dt, enemySpatialIndex = null) {
  ...
  for (const saw of state.launchedSaws) {
    ...
    for (let candidateIndex = 0; candidateIndex < candidateCount; candidateIndex += 1) {
      ...
      const dx = saw.x - enemy.x;
      const dy = saw.y - enemy.y;
      const reach = cfg.radius + enemy.radius;
...
function updateProjectiles(state, dt, enemySpatialIndex = null) {
  const { width, height } = GAME_CONFIG.arena;
  for (const projectile of state.projectiles) {
    projectile.x += projectile.vx * dt;
    projectile.y += projectile.vy * dt;
    projectile.life -= dt;
    const nearby = enemySpatialIndex ? queryEnemySpatialIndex(enemySpatialIndex, projectile.x, projectile.y, projectile.radius) : null;
    const candidateCount = nearby ? nearby.length : state.enemies.length;
    for (let candidateIndex = 0; candidateIndex < candidateCount; candidateIndex += 1) {
      const enemy = state.enemies[nearby ? nearby[candidateIndex] : candidateIndex];
      ...
      if (dx * dx + dy * dy <= reach * reach) {
        ...
        if (projectile.splash) {
          for (const nearby of state.enemies) if (nearby !== enemy && nearby.hp > 0 && Math.hypot(nearby.x - enemy.x, nearby.y - enemy.y) <= projectile.splash + nearby.radius) damageEnemy(state, nearby, damage * 0.55, "cannonEcho");
          state.elementFx.push({ element: "cannonEcho", x: enemy.x, y: enemy.y, radius: projectile.splash, life: 0.35, maxLife: 0.35 });
        }
...
function findDroneDetonationTarget(state, drone) {
  ...
  for (const enemy of state.enemies) {
    if (enemy.hp <= 0) continue;
    const enemyPriority = priority(enemy);
    const distance = Math.hypot(enemy.x - drone.x, enemy.y - drone.y);
    if (enemyPriority < bestPriority || (enemyPriority === bestPriority && (distance < bestDistance || (distance === bestDistance && enemy.id < best.id)))) {
      best = enemy;
...
function findDroneHuntTarget(enemies, drone) {
  let best = null;
  let bestMarked = Infinity;
  let bestDistance = Infinity;
  for (const enemy of enemies) {
    if (enemy.hp <= 0 || !enemy.elite) continue;
    const marked = Number(enemy.markTimer > 0);
    const distance = Math.hypot(enemy.x - drone.x, enemy.y - drone.y);
    if (marked < bestMarked || (marked === bestMarked && (distance < bestDistance || (distance === bestDistance && enemy.id < best.id)))) {
      best = enemy;
      bestMarked = marked;
      bestDistance = distance;
    }
...
function updateDrones(state, dt) {
  ...
  const service = modulesAdjacent(state, "hangar", "service") ? installedModule(state, "service") : null;
  const serviceRule = MODULE_BALANCE.service;
  const returnMultiplier = movementMultiplier * (service?.level >= 2 ? serviceRule.returnMultiplier : 1);
  if (service) { service.launchBuff = Math.max(0, (service.launchBuff ?? 0) - dt); service.servicing = false; }
  for (const drone of state.drones) {
    const wasRecovering = drone.recoveryTimer > 0;
    drone.recoveryTimer = Math.max(0, (drone.recoveryTimer ?? 0) - dt);
  ...
  if (detonateMode && state.tower.droneEnergy < cfg.detonate.energyCost) {
    ...
    const flightMultiplier = service?.launchBuff > 0 ? serviceRule.flightDrainMultiplier : 1;
    state.tower.droneEnergy = Math.max(0, state.tower.droneEnergy - (hangar ? tuning.attackDrain : cfg.attackDrainPerSecond) * flightMultiplier * dt);
    ...
    const relayMultiplier = 1 + (state.tower.upgrades.droneRelay ?? 0) * tech.relayRegenPerLevel;
    const docked = service ? state.drones.filter((drone, index) => {
      const home = getDroneOrbitPosition(state, index);
      return drone.recoveryTimer <= 0 && Math.hypot(drone.x - home.x, drone.y - home.y) <= serviceRule.dockRadius;
    }).length : 0;
    ...
    if (service && docked > 0 && state.tower.droneEnergy > before) {
      ...
      if (state.tower.droneEnergy >= maxEnergy && service.recoveredEnergy >= maxEnergy * .1) service.recharged = true;
      service.pulseCooldown = (service.pulseCooldown ?? 0) - dt;
      if (service.pulseCooldown <= 0) { state.events.push({ type: "moduleService" }); service.pulseCooldown = .4; }
    }
  ...
  if (hasEndlessRelic(state, "droneDuplex") && !detonateMode && state.tower.droneMode === "collect" && count > 0) {
    state.endlessShop.droneSupportTimer = Math.max(0, (state.endlessShop.droneSupportTimer ?? 0) - dt);
    if (state.endlessShop.droneSupportTimer <= 0) {
      const marked = state.enemies.find((enemy) => enemy.hp > 0 && state.tower.priorityTargetIds.includes(enemy.id))
        ?? rankTargets(state, state.enemies.filter((enemy) => enemy.hp > 0), 1)[0];
      if (marked) {
        damageEnemy(state, marked, getTowerStats(state).damage * cfg.damageMultiplier, "drone");
        const droneIndex = Math.floor(state.time / ENDLESS_SHOP_RULES.droneSupportInterval) % count;
        const drone = state.drones[droneIndex];
        state.events.push({ type: "droneSupport", droneIndex, x1: drone?.x ?? marked.x, y1: drone?.y ?? marked.y, x2: marked.x, y2: marked.y, targetId: marked.id });
        state.endlessShop.droneSupportTimer = ENDLESS_SHOP_RULES.droneSupportInterval;
      }
    }
  }
  ...
  for (let index = 0; index < state.drones.length; index += 1) {
    ...
    if (detonateMode && state.tower.droneDetonateActive) {
      ...
      if (!target) {
        drone.targetId = null;
        moveDroneTowards(drone, getDroneOrbitPosition(state, index), cfg.returnSpeed * returnMultiplier, dt);
        continue;
      }
      drone.targetId = target.id;
      const distance = moveDroneTowards(drone, target, cfg.attackSpeed * movementMultiplier, dt);
    ...
    const huntTarget = state.tower.upgrades.droneHunt > 0 ? findDroneHuntTarget(living, drone) : null;
    const heavyTarget = hasSpecialization(state, "hangarHeavy") ? living.filter(enemy => isBossEnemy(enemy) || enemy.elite).sort((a,b) => Number(isBossEnemy(b)) - Number(isBossEnemy(a)) || a.id - b.id)[0] : null;
    const claimed = hasSpecialization(state, "hangarSwarm") ? new Set(state.drones.slice(0, index).map(other => other.targetId)) : null;
    const swarmPool = claimed ? living.filter(enemy => !claimed.has(enemy.id)) : [];
    const target = lockedAnchor ?? heavyTarget ?? (claimed ? rankTargets(state, swarmPool.length ? swarmPool : living, 1)[0] : huntTarget ?? rankTargets(state, living, 1)[0]);
    if (!target) {
      drone.targetId = null;
      moveDroneTowards(drone, getDroneOrbitPosition(state, index), cfg.returnSpeed * returnMultiplier, dt);
      continue;
...
export function useSkill(state, key, options = {}) {
  ...
  if (key === "overload" && hasEndlessRelic(state, "perpetualOverload") && skill.permanentEngaged) {
  ...
  if (key === "heal") {
    ...
    skill.permanentEngaged = hasEndlessRelic(state, "perpetualOverload");
    skill.active = skill.permanentEngaged ? 0 : config.duration * (hasSkillResearchNode(state, "overload", "stabilizer") ? GAME_CONFIG.activeSkillResearch.overload.durationMultiplier : 1);
    skill.heat = 0;
    skill.slow = 0;
    skill.pulse = 0;
    ...
    const globalProtocol = hasEndlessRelic(state, "globalStarfall");
    const requestedAngle = Number(options.angle);
    if ((!globalProtocol && !Number.isFinite(requestedAngle)) || !state.enemies.some((enemy) => enemy.hp > 0)) return false;
    const research = GAME_CONFIG.activeSkillResearch.starfall;
    const firstTarget = state.enemies.find((enemy) => enemy.hp > 0);
    const fallbackAngle = firstTarget ? Math.atan2(firstTarget.y - getTowerPosition(state).y, firstTarget.x - getTowerPosition(state).x) : 0;
    const angle = globalProtocol ? fallbackAngle : Math.atan2(Math.sin(requestedAngle), Math.cos(requestedAngle));
    const coneHalfAngle = globalProtocol ? Math.PI : getStarfallConeHalfAngle(state);
    ...
    for (const enemy of state.enemies) {
      ...
      if (enemy.hp <= 0 || (!globalProtocol && angleDistance(enemyAngle, angle) > coneHalfAngle)) continue;
    ...
    skill.aiming = false;
    skill.protocol = globalProtocol ? "global" : "manual";
...
export function calculateRunScore(state) {
  ...
  const coinBasis = Math.max(0, Math.floor(state.coins + (state.endlessShop?.spent ?? 0) * 0.5));
  const coinBonus = coinBasis * GAME_CONFIG.score.coinMultiplier;
...
export function updateGame(state, dt = GAME_CONFIG.fixedStep) {
  ...
  if ((state.endlessShop?.levels?.autoCoinVacuum ?? 0) > 0 && state.endlessShop.autoCoinEnabled && state.skills.coinVacuum.cooldown <= 0) {
    let coinCount = 0;
    let coinValue = 0;
    for (const orb of state.coinOrbs) {
      if (orb.expired || orb.collected) continue;
      coinCount += orb.pileCount ?? 1;
      coinValue += orb.value ?? 0;
      if (coinCount >= ENDLESS_SHOP_RULES.coinCountThreshold || coinValue >= ENDLESS_SHOP_RULES.coinValueThreshold) break;
    }
    if (coinCount >= ENDLESS_SHOP_RULES.coinCountThreshold || coinValue >= ENDLESS_SHOP_RULES.coinValueThreshold) useSkill(state, "coinVacuum", { automatic: true });
  }
  ...
  state.tower.damageImmunity = Math.max(0, (state.tower.damageImmunity ?? 0) - dt);
  ...
  overloadSkill.unstable = Math.max(0, (overloadSkill.unstable ?? 0) - dt);
  if (hasEndlessRelic(state, "perpetualOverload") && overloadSkill.permanentEngaged) {
    const heatMultiplier = hasSkillResearchNode(state, "overload", "stabilizer") ? GAME_CONFIG.activeSkillResearch.overload.heatGainMultiplier : 1;
    ...
    if (overloadSkill.heat >= GAME_CONFIG.skills.overload.heatCap) {
      releaseOverloadPulse(state);
      overloadSkill.heat = 30;
      overloadSkill.slow = 0;
      overloadSkill.unstable = ENDLESS_SHOP_RULES.overloadUnstableDuration;
    }
  } else if (overloadSkill.active > 0) {
  ...
  const aimClaimed = new Set();
  for (const gun of guns) {
    const gunKey = gun?.id ?? "base";
    const heavyGun = gun?.id === "cannon";
    const pool = state.enemies.filter((enemy) => {
      if (enemy.hp <= 0) return false;
      const distance = Math.hypot(enemy.x - towerPos.x, enemy.y - towerPos.y);
      return heavyGun ? distance <= 620 && distance >= 140 : distance <= baseRange;
    });
    const free = pool.filter((enemy) => !aimClaimed.has(enemy.id));
    const aimTarget = rankTargets(state, free.length ? free : pool, 1)[0] ?? null;
    if (aimTarget) aimClaimed.add(aimTarget.id);
    state.tower.gunAimTargetIds[gunKey] = aimTarget?.id ?? null;
  }
  const fireClaimed = new Set();
  ...
  if (!entryCombatLocked && (gun ? gun.cooldown : state.tower.fireCooldown) <= 0 && fireTower(state, gun?.id, fireClaimed)) {
  ...
  if (!entryCombatLocked) {
    ...
    updateSaws(state, dt, enemySpatialIndex);
    updateLaunchedSaws(state, dt, enemySpatialIndex);
    updateSawGuns(state, dt);
    updateProjectiles(state, dt, enemySpatialIndex);
    if (hasSpecialization(state, "bladeGuard") && !(state.tower.bladeBlockCooldown > 0)) {
      const tower = getTowerPosition(state);
      const index = state.hostileProjectiles.findIndex(shot => Math.hypot(shot.x - tower.x, shot.y - tower.y) <= getSawOrbitRadius(state) + 28);
      if (index >= 0) {
        state.hostileProjectiles.splice(index, 1);
        state.tower.bladeBlockCooldown = 1.2;
        state.tower.bladeExpansion = 2;
        state.events.push({ type: "moduleSynergy", name: "晶刃屏障 · 炮弹已拦截" });
      }
    }
  ...
  resolveDeaths(state);
  releaseEndlessShopNotice(state);
