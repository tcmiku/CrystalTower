...
七种第一章常见小怪依据 `assets/generated/enemy-atlas.png` 重建：暗紫岩甲、熔火裂隙、红晶轮廓。剪影遵循美术方向——浮灵圆钝爪形、疾行兽锐角箭形、重甲/守卫宽重多边形、咒晶怪紫晶环绕、冲撞兽橙热撞角。
...
- 预览：`/enemy-model-preview.html`，下拉切换七种并环视。
...
模块化晶塔采用完整圆形塔身、分层装甲、中央晶核塔冠和能量环。模块以短支架和弧形固定座挂在塔壁外侧；六格仅用于装配管理，不对应实体甲板或平台。晶核与分段能量环具有悬浮和旋转动画。轻炮、重炮的活动炮组与固定底盘分离，按各自开火事件播放后坐与炮口闪光。`module-model-preview.html` 的“演示主炮攻击”可查看动作。
