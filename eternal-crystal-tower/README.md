...
防御塔已改为实体三维网格，使用 WebGL 绘制并合成到战场 Canvas。打开 [四阶模型检视](http://127.0.0.1:4173/tower-model-preview.html) 可旋转查看、切换炮膛/元素/过载状态，并下载各阶 GLB。建模源与导出说明见 [模型说明](assets/models/README.md)。
...
第一章现采用六格模块拼装。精英奖励持续提供武器专精、遗物替换和补给；冰火融爆、冰雷碎晶、火雷电链各有不同效果。巨兽与终局首领增加连接节点，怪潮沿地图四处入口编队进攻，护盾和机库可与相邻武器协作。新增预警、清场整备、晶簇共鸣和无人机远征，伤害、射速、升阶集中到装配页顶部。当前刷怪与操作规则见 [地图玩法落地说明](design/MAP_ASSAULT_IMPLEMENTATION.md)。完整规则见 [第一章玩法说明](design/CHAPTER_ONE_GAMEPLAY.md)。

| 试玩参数 | 内容 |
| --- | --- |
| `?preview=chapter-one-rewards&seed=61` | 满槽遗物替换、武器专精与连续奖励 |
| `?preview=chapter-one-boss&seed=61` | 首领连接节点 |
